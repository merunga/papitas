...
Boot.prototype = {
  preload: function() {
    ...
    this.load.onFileComplete.add(function(percent, asset) {
      if(asset == 'cortina') {
        var cortinaDeFondo = Experto.sonidos.cortinaDeFondo = Experto.game.add.audio('cortina',1,true);
        cortinaDeFondo.play();
      } /*else if(asset == 'efectos') {
        var efectosDeSonido = Experto.sonidos.efectosDeSonido = Experto.game.add.audio('efectos');
        efectosDeSonido.addMarker('movimiento', 0.16, 0.232);
      } */else if(asset == 'finalEtapa') {
        Experto.sonidos.finalEtapa = Experto.game.add.audio('finalEtapa',8);
      }
    });
  ...
  create: function() {
    Experto.game.input.maxPointers = 1;
  }
};
